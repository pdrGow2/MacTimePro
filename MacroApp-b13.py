...
# Função auxiliar para adicionar efeito hover aos botões
def add_hover_effect(button, normal_bg, hover_bg):
    button.bind("<Enter>", lambda e: button.config(bg=hover_bg), add="+")
    button.bind("<Leave>", lambda e: button.config(bg=normal_bg), add="+")
...
class MacroApp:
    def __init__(self, root):
        ...
        # Tenta carregar os ícones (ajuste os caminhos conforme necessário)
        self.icon_capturar = load_icon("icones/icone_capturar.png")
        self.icon_texto    = load_icon("icones/icone_texto.png")
        self.icon_tecla    = load_icon("icones/icone_tecla.png")
        self.icon_espera   = load_icon("icones/icone_espera.png")
        self.icon_apagar   = load_icon("icones/icone_apagar.png")
        self.icon_carregar = load_icon("icones/icone_carregar.png")
        self.icon_importar = load_icon("icones/icone_importar.png")
        self.icon_exportar = load_icon("icones/icone_exportar.png")
        self.icon_executar = load_icon("icones/icone_executar.png")
        self.icon_parar    = load_icon("icones/icone_parar.png")
        self.icon_about    = load_icon("icones/icone_about.png")
        
        # Frame dos botões
        button_frame = tk.Frame(root, bg="#282828")
        button_frame.pack(fill=tk.X, pady=5)
        inner_frame = tk.Frame(button_frame, bg="#282828")
        inner_frame.pack(anchor="center")
        
        # Função para criar botão: se ícone existir, usa-o (sem width/height) senão usa emoji com width=5 e height=2.
        def create_button(parent, icon, fallback, cmd, tip, bg="#535353", fg="white"):
            if icon is not None:
                btn = tk.Button(parent, image=icon, command=cmd,
                                relief=tk.FLAT, bd=0, bg=bg)
            else:
                btn = tk.Button(parent, text=fallback, command=cmd, font=("Segoe UI Emoji", 14),
                                width=5, height=2, relief=tk.FLAT, bd=0, bg=bg, fg=fg)
            CreateToolTip(btn, tip)
            add_hover_effect(btn, normal_bg=bg, hover_bg="#6e6e6e")
            return btn
        
        # Botões de ação (fundo #535353)
        self.btn_capturar = create_button(inner_frame, self.icon_capturar, "🖱️", self.capture_mouse_click, "Captura um clique do mouse")
        self.btn_capturar.pack(side=tk.LEFT, padx=2)
        
        self.btn_texto = create_button(inner_frame, self.icon_texto, "📝", self.add_text_event, "Adiciona um bloco de texto à timeline")
        self.btn_texto.pack(side=tk.LEFT, padx=2)
        
        self.btn_tecla = create_button(inner_frame, self.icon_tecla, "⌨️", self.add_key_event, "Adiciona uma sequência de teclas")
        self.btn_tecla.pack(side=tk.LEFT, padx=2)
        
        self.btn_espera = create_button(inner_frame, self.icon_espera, "⏱️", self.add_wait_event, "Adiciona um tempo de espera")
        self.btn_espera.pack(side=tk.LEFT, padx=2)
        
        self.btn_apagar = create_button(inner_frame, self.icon_apagar, "🗑️", self.add_clear_event, "Adiciona o evento de apagar campo")
        self.btn_apagar.pack(side=tk.LEFT, padx=2)
        
        self.btn_carregar = create_button(inner_frame, self.icon_carregar, "📋", self.load_timeline, "Carrega uma lista de um arquivo TXT")
        self.btn_carregar.pack(side=tk.LEFT, padx=2)
        
        self.btn_importar = create_button(inner_frame, self.icon_importar, "📥", self.import_timeline, "Importa uma timeline de arquivo")
        self.btn_importar.pack(side=tk.LEFT, padx=2)
        
        self.btn_exportar = create_button(inner_frame, self.icon_exportar, "📤", self.export_timeline, "Exporta a timeline para arquivo")
        self.btn_exportar.pack(side=tk.LEFT, padx=2)
        
        self.btn_about = create_button(inner_frame, self.icon_about, "❓", self.show_about, "Sobre este sistema")
        self.btn_about.pack(side=tk.LEFT, padx=2)
        
        # Botão Executar e seu menu suspenso
        exec_frame = tk.Frame(inner_frame, bg="#282828")
        exec_frame.pack(side=tk.LEFT, padx=2)
        if self.icon_executar is not None:
            self.btn_executar = tk.Button(exec_frame, image=self.icon_executar, command=self.execute_timeline,
                                          relief=tk.FLAT, bd=0, bg="green")
        else:
            self.btn_executar = tk.Button(exec_frame, text="▶️", command=self.execute_timeline,
                                          font=("Segoe UI Emoji", 14), width=5, height=2,
                                          relief=tk.FLAT, bd=0, bg="green", fg="white")
        self.btn_executar.pack(side=tk.LEFT, padx=2)
        CreateToolTip(self.btn_executar, "Executa a macro")
        add_hover_effect(self.btn_executar, normal_bg="green", hover_bg="#2E8B57")
        
        # Botão de seta para executar em loop; usamos texto e mantemos tamanho similar
        self.loop_mb = tk.Menubutton(exec_frame, text="▼", relief=tk.FLAT, bd=0,
                                      bg="green", fg="white", font=("Segoe UI Emoji", 14), width=5, height=2)
        loop_menu = tk.Menu(self.loop_mb, tearoff=0)
        loop_menu.add_command(label="Executar em loop", command=self.execute_timeline_loop)
        self.loop_mb.config(menu=loop_menu)
        self.loop_mb.pack(side=tk.LEFT, padx=2)
        CreateToolTip(self.loop_mb, "Executa a macro em loop")
        add_hover_effect(self.loop_mb, normal_bg="green", hover_bg="#2E8B57")
        
        # Botão Parar (vermelho)
        if self.icon_parar is not None:
            self.btn_parar = tk.Button(inner_frame, image=self.icon_parar, command=self.stop_execution,
                                       relief=tk.FLAT, bd=0, bg="red")
        else:
            self.btn_parar = tk.Button(inner_frame, text="⏹️", command=self.stop_execution,
                                       font=("Segoe UI Emoji", 14), width=5, height=2,
                                       relief=tk.FLAT, bd=0, bg="red", fg="white")
        self.btn_parar.pack(side=tk.LEFT, padx=2)
        CreateToolTip(self.btn_parar, "Interrompe a macro")
        add_hover_effect(self.btn_parar, normal_bg="red", hover_bg="#B22222")
        
        # Frame para a timeline
        timeline_container = tk.Frame(root, bg="#282828")

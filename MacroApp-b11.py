...
class MacroApp:
    def __init__(self, root):
        ...
        tk.Button(button_frame, text="Capturar Clique", command=self.capture_mouse_click)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Adicionar Texto", command=self.add_text_event)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Adicionar Tecla", command=self.add_key_event)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Adicionar Espera", command=self.add_wait_event)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Apagar Campo", command=self.add_clear_event)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        
        tk.Button(button_frame, text="Carregar Lista TXT", command=self.load_timeline)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Importar Timeline", bg="blue", fg="white", command=self.import_timeline)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        tk.Button(button_frame, text="Exportar Timeline", bg="purple", fg="white", command=self.export_timeline)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        
        # Widget composto: botão "Executar" com seta para o menu suspenso "Executar em loop"
        exec_frame = tk.Frame(button_frame)
        exec_frame.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        self.exec_button = tk.Button(exec_frame, text="Executar", bg="green", fg="white", command=self.execute_timeline)
        self.exec_button.pack(side=tk.LEFT, fill=tk.X, expand=True)
        loop_mb = tk.Menubutton(exec_frame, text="▼", relief=tk.RAISED, width=2, bg="green", fg="white")
        ...
        loop_mb.pack(side=tk.LEFT)
        
        tk.Button(button_frame, text="Parar", bg="red", fg="white", command=self.stop_execution)\
            .pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
    ...
    def move_selected_up(self):
        for i in range(1, len(self.events)):
            if self.events[i].selected_var.get() and not self.events[i-1].selected_var.get():
                self.events[i], self.events[i-1] = self.events[i-1], self.events[i]
        self.refresh_timeline()
    
    def move_selected_down(self):
        for i in range(len(self.events)-2, -1, -1):
            if self.events[i].selected_var.get() and not self.events[i+1].selected_var.get():
                self.events[i], self.events[i+1] = self.events[i+1], self.events[i]
        self.refresh_timeline()
    
    def select_event(self, event):
        if self.selected_event and self.selected_event.winfo_exists():
    ...
    def deselect_event(self, event):
        if self.selected_event and self.selected_event.winfo_exists() and event.widget == self.canvas:
            self.selected_event.config(bg="lightgray")
            self.selected_event = None
    ...
    def show_countdown_and_execute(self, required_files, current_loop=None, total_loops=None):
        ...
        countdown_window.geometry(f"{win_width}x{win_height}+{x}+{y}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = MacroApp(root)
    root.bind("<Delete>", lambda event: app.delete_selected_event(event))
    root.mainloop()

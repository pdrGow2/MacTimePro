...
# MÉTODOS DE CAPTURA E ADIÇÃO (mantidos inalterados)
    def capture_mouse_click(self):
        ...
        self.center_window(cap_win, 400, 200)
        cap_win.attributes("-topmost", True)
        cap_win.configure(bg="#282828")
        label = tk.Label(cap_win, text="Mova o mouse até a posição desejada e aperte ENTER", padx=20, pady=10,
                         bg="#282828", fg="white")
        label.pack(expand=True, fill=tk.BOTH)
        radio_frame = tk.Frame(cap_win, bg="#282828")
        radio_frame.pack(pady=5)
        click_type_var = tk.StringVar(value="Clique Simples")
        type_label = tk.Label(cap_win, text="Opção selecionada: Clique Simples", bg="#282828", fg="white")
        type_label.pack()
        def update_type():
            type_label.config(text=f"Opção selecionada: {click_type_var.get()}")
            cap_win.after(100, update_type)
        update_type()
        tk.Radiobutton(radio_frame, text="Clique Simples", variable=click_type_var, value="Clique Simples",
                       bg="#282828", fg="white", relief=tk.FLAT, indicatoron=True,selectcolor="#444444").pack(anchor="w")
        tk.Radiobutton(radio_frame, text="Duplo Clique", variable=click_type_var, value="Duplo Clique",
                       bg="#282828", fg="white", relief=tk.FLAT, indicatoron=True,selectcolor="#444444").pack(anchor="w")
        tk.Radiobutton(radio_frame, text="Botão Direito", variable=click_type_var, value="Botão Direito",
                       bg="#282828", fg="white", relief=tk.FLAT, indicatoron=True,selectcolor="#444444").pack(anchor="w")
        tk.Radiobutton(radio_frame, text="Scroll", variable=click_type_var, value="Scroll",
                       bg="#282828", fg="white", relief=tk.FLAT, indicatoron=True,selectcolor="#444444").pack(anchor="w")
        def update_position():
            x = self.root.winfo_pointerx()
            y = self.root.winfo_pointery()
        ...
        def on_enter(e):
            ...
            y = self.root.winfo_pointery()
            cap_win.destroy()
            self.add_event(f"{click_type_var.get()} em ({x}, {y})")
    ...
    def add_key_event(self):
        ...
        self.center_window(key_win, 400, 200)
